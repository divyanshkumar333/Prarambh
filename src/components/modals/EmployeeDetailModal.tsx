...
const statusStyle = (s: Task['status']) => ({
  done:        { chip: 'bg-green-100 text-emerald-400 border-emerald-500/20 hover:bg-green-200',       icon: <CheckCircle size={13} className="text-green-500 flex-shrink-0" />,             label: 'Done'        },
  'in-progress':{ chip: 'bg-blue-100 text-blue-400 border-blue-500/20 hover:bg-blue-200',         icon: <Clock size={13} className="text-blue-500 flex-shrink-0" />,                   label: 'In Progress' },
  pending:     { chip: 'bg-surface-elevated/50 text-text-secondary border-border hover:bg-surface-elevated',       icon: <div className="w-3 h-3 rounded-full border-2 border-border flex-shrink-0" />, label: 'Pending'     },
}[s])

const priorityStyle = (p?: Task['priority']) => {
  if (p === 'high')   return 'bg-red-500/10 text-red-400 border-red-500/20 hover:bg-red-100'
  if (p === 'medium') return 'bg-orange-50 text-orange-600 border-orange-200 hover:bg-orange-100'
  if (p === 'low')    return 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20 hover:bg-green-100'
  return 'bg-surface-elevated text-text-secondary/70 border-border hover:bg-surface-elevated/50'
}
...
export default function EmployeeDetailModal({ employee, onClose }: Props) {
  ...
  return (
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4" onClick={onClose}>
      ...
      <div
        className="relative bg-surface rounded-2xl shadow-2xl w-full max-w-2xl border border-border max-h-[92vh] overflow-y-auto"
        ...
        <div className="px-6 py-5 flex items-center justify-between rounded-t-2xl">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-text-secondary text-sm">{employee.role} · {employee.team}</p>
          ...
          <button onClick={onClose} className="p-2 rounded-lg hover:bg-surface/60 text-text-secondary transition-colors">
            <X size={18} />
          </button>
        </div>
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              ...
              <div key={item.label} className="bg-surface-elevated rounded-xl p-3 border border-border">
          ...
          {/* Progress bg-surface-elevated border border-border rounded-2xl p-6 shadow-sm */}
          <div className="bg-surface-elevated border border-border rounded-2xl p-6 shadow-sm p-5">
            <div className="flex items-center justify-between mb-3">
              <h3 className="font-bold text-text-primary">Prarambh Progress</h3>
              <span className="text-text-secondary font-bold text-lg">{rate}%</span>
            </div>
            ...
            <div className="grid grid-cols-3 gap-3 text-center">
              {[
                { label: 'Completed',   count: done.length,    color: 'text-emerald-400', bg: 'bg-emerald-500/10',  border: 'border-green-100' },
                { label: 'In Progress', count: inProg.length,  color: 'text-blue-400',  bg: 'bg-blue-500/10',   border: 'border-blue-100'  },
                { label: 'Pending',     count: pending.length, color: 'text-text-secondary', bg: 'bg-surface-elevated',  border: 'border-border' },
              ].map(s => (
          ...
          {/* Bio */}
          {employee.bio && (
            <div className="bg-blue-500/10 border border-blue-100 rounded-xl p-4 flex gap-3">
              <MessageSquare size={16} className="text-blue-500 flex-shrink-0 mt-0.5" />
              <div>
                <p className="text-xs font-semibold text-blue-400 mb-0.5">About {employee.name.split(' ')[0]}</p>
                <p className="text-sm text-blue-400 leading-relaxed">{employee.bio}</p>
              </div>
            </div>
          )}
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <h3 className="font-bold text-text-primary">Assigned Tasks</h3>
              <div className="flex items-center gap-2">
                <span className="text-xs text-text-secondary font-medium">{rate}% complete · {myTasks.length} total</span>
                <button
            ...
            {myTasks.length === 0 ? (
              <div className="text-center py-10 bg-surface-elevated rounded-xl border border-dashed border-border">
                <FileText size={28} className="mx-auto mb-2 text-text-secondary/50" />
                <p className="text-sm text-text-secondary mb-3">No tasks assigned yet</p>
              ...
              <div className="space-y-2 max-h-[420px] overflow-y-auto pr-1">
                {tasksWithOrder.map((task, idx) => {
                  ...
                  return (
                    <div key={task.id} className={`rounded-xl border bg-surface overflow-hidden transition-all ${isPendingRemove ? 'border-red-300' : 'border-border'}`}>
                      ...
                      <div className="flex items-center gap-2 p-3">
                        ...
                        <div className="flex flex-col gap-0.5 flex-shrink-0">
                          <button
                            ...
                            disabled={isFirst}
                            className="p-0.5 rounded text-text-secondary/50 hover:text-text-secondary hover:bg-surface-elevated disabled:opacity-20 disabled:cursor-not-allowed transition-colors"
                            title="Move up"
                          >
                            <ArrowUp size={12} />
                          </button>
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-1.5 flex-wrap">
                            <p className={`text-sm font-medium ${task.status === 'done' ? 'line-through text-text-secondary/70' : 'text-text-primary'}`}>
                              {task.title}
                            </p>
                            {task.requiresInput && (
                          ...
                          <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                            <span className="text-xs text-text-secondary/70">{task.category} · {task.estimatedTime}</span>
                            <span className={`text-xs font-semibold px-1.5 py-0.5 rounded-full border flex-shrink-0
                              ${task.assignedBy === 'admin'  ? 'bg-blue-500/10 text-blue-400 border-blue-100' :
                        ...
                        {hasExtra && (
                          <button
                            ...
                            className="p-1 rounded hover:bg-surface-elevated text-text-secondary/70 flex-shrink-0 transition-colors"
                          >
                        ...
                        {task.playgroundEnabled && task.playgroundState?.lastSaved && (
                          <button
                            ...
                            className="p-1 rounded flex-shrink-0 text-teal-400 hover:text-brand-400 hover:bg-brand-500/10 transition-colors"
                          >
                        ...
                        {canEditTask(task) && (
                          <button
                            ...
                            title="Edit task"
                            className="p-1 rounded flex-shrink-0 text-text-secondary/70 hover:text-text-primary hover:bg-surface-elevated/50 transition-colors"
                          >
                            <Edit3 size={13} />
                          </button>
                        )}
                        ...
                        <button
                          ...
                          title="Remove task"
                          className={`p-1 rounded flex-shrink-0 transition-colors ${isPendingRemove ? 'text-red-400 bg-red-500/10' : 'text-red-300 hover:text-red-400 hover:bg-red-500/10'}`}
                        >
                          <Trash2 size={13} />
                        </button>
                      </div>
                      ...
                      {isPendingRemove && canEdit(task) && (
                        <div className="bg-red-500/10 border-t border-red-500/20 px-4 py-2.5 flex items-center justify-between gap-3">
                          <p className="text-xs text-red-400 font-medium">Remove this task? This cannot be undone.</p>
                          <div className="flex gap-2 flex-shrink-0">
                            <button onClick={() => setConfirmRemoveTask(null)} className="text-xs px-2.5 py-1 rounded-lg border border-border bg-surface text-text-secondary hover:bg-surface-elevated transition-colors">Cancel</button>
                      ...
                      {task.status === 'done' && (
                        <div className="border-t border-green-100 px-4 pb-3 pt-2.5 bg-emerald-500/10/30 space-y-2">
                          {/* Existing feedback */}
                          {(task.feedback ?? [])
                            .filter(fb => fb.visibility.includes(currentRole))
                            .map(fb => (
                              <div key={fb.id} className="bg-surface border border-green-100 rounded-lg p-2.5">
                                <div className="flex items-center gap-2 mb-1 flex-wrap">
                                  <span className={`text-xs font-semibold px-1.5 py-0.5 rounded-full border
                                    ${fb.addedByRole === 'admin' ? 'bg-blue-500/10 text-blue-400 border-blue-100' :
                                      fb.addedByRole === 'hr' ? 'bg-purple-50 text-purple-700 border-purple-100' :
                                      fb.addedByRole === 'mentor' ? 'bg-brand-500/10 text-brand-400 border-brand-500/20' :
                                      'bg-surface text-text-secondary border-border'}`}>
                                    {fb.addedByRole === 'admin' ? '🔑' : fb.addedByRole === 'hr' ? '👔' : fb.addedByRole === 'mentor' ? '🤝' : '👤'} {fb.addedBy}
                                  </span>
                                  <span className="text-xs text-text-secondary/70">{new Date(fb.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</span>
                                  <span className="ml-auto flex items-center gap-1 text-xs text-text-secondary/70">
                          ...
                          {canEditTask(task) && (feedbackOpen[task.id] ? (
                            <div className="space-y-2">
                              <textarea
                                className="w-full text-xs border border-emerald-500/20 rounded-lg p-2 resize-none focus:outline-none focus:ring-1 focus:ring-green-400 bg-surface"
                                rows={2}
                              ...
                              {/* Visibility picker */}
                              <div>
                                <p className="text-xs text-text-secondary font-semibold mb-1 flex items-center gap-1"><Eye size={10} />Visible to:</p>
                                  ...
                                  {VISIBILITY_ROLES.map(role => {
                                    ...
                                    return (
                                      <button
                                        ...
                                        className={`text-xs px-2 py-0.5 rounded-full border transition-colors ${selected ? 'bg-green-600 text-white border-green-600' : 'bg-surface text-text-secondary border-border hover:border-green-400'}`}
                                      >
                                        {ROLE_LABELS[role]}
                                      </button>
                                    )
                              ...
                              <div className="flex gap-2">
                                <button onClick={() => setFeedbackOpen(p => ({ ...p, [task.id]: false }))} className="text-xs px-3 py-1.5 rounded-lg border border-border text-text-secondary hover:bg-surface-elevated transition-colors">Cancel</button>
                            ...
                            <button
                              ...
                              className="text-xs font-medium text-emerald-400 hover:text-green-900 flex items-center gap-1 transition-colors"
                            >
                      ...
                      {/* Expanded details */}
                      {isExpanded && hasExtra && (
                        <div className="border-t border-border px-4 pb-3 pt-2.5 space-y-3 bg-surface-elevated/40">
                          {hasSubs && (
                            <div>
                              <p className="text-xs font-semibold text-text-secondary mb-1.5">Subtasks</p>
                                ...
                                {(task.subtasks ?? []).map(sub => (
                                  <div key={sub.id} className="flex items-center gap-2">
                                    <button
                                      ...
                                      className={`w-4 h-4 rounded border-2 flex items-center justify-center flex-shrink-0 transition-colors ${sub.status === 'done' ? 'bg-green-500 border-green-500' : 'border-border hover:border-brown-500'}`}
                                    >
                                      {sub.status === 'done' && <CheckCircle size={10} className="text-white" />}
                                    </button>
                                    <span className={`text-xs ${sub.status === 'done' ? 'line-through text-text-secondary/70' : 'text-text-primary'}`}>{sub.title}</span>
                                  </div>
                                ))}
                              </div>
                            </div>
                          ...
                          {hasDocs && (
                            <div>
                              <p className="text-xs font-semibold text-text-secondary mb-1">Supporting Documents</p>
                              {(task.supportingDocs ?? []).map(dId => {
                                const doc = state.documents.find(d => d.id === dId)
                                return doc ? (
                                  <div key={dId} className="flex items-center gap-1.5 text-xs text-text-secondary">
                          ...
                          {hasLinks && (
                            <div>
                              <p className="text-xs font-semibold text-text-secondary mb-1">Links</p>
                              {(task.supportingLinks ?? []).map((lnk, i) => (
                                <a key={i} href={lnk.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 text-xs text-blue-400 hover:underline">
                          ...
                          {task.requiresInput && (
                            <div className="bg-purple-50 border border-purple-100 rounded-lg p-2.5">
                              ...
                              {task.inputValue
                                ? <div className="bg-surface border border-purple-200 rounded p-2 text-xs text-text-primary">{task.inputValue}</div>
          ...
          {employee.resumeFileName && (
            <div className="flex items-center gap-3 p-4 bg-surface-elevated rounded-xl border border-border">
              <FileText size={18} className="text-text-secondary flex-shrink-0" />
              <div className="flex-1">
                <p className="text-sm font-semibold text-text-primary">Resume on file</p>
                <p className="text-xs text-text-secondary/70 mt-0.5">{employee.resumeFileName}</p>
              </div>
          ...
          {!showConfirm ? (
            <button
              ...
              className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl border border-red-500/20 text-red-500 hover:bg-red-500/10 hover:border-red-400 transition-colors text-sm font-semibold"
            >
