...
export default function AssignTaskModal({ tasks, onClose, preselectedEmployeeId, assignedBy, assignedByName }: Props) {
  ...
  if (saved) return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-surface rounded-2xl p-10 text-center shadow-2xl max-w-sm w-full animate-bounce-in">
        ...
        <h3 className="text-xl font-bold text-text-primary mb-2">{selectedTaskIds.length} Tasks Assigned!</h3>
        <p className="text-text-secondary text-sm">The employee will see these tasks in their dashboard.</p>
      </div>
    </div>
  )
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto">
      <div className="bg-surface rounded-2xl shadow-2xl w-full max-w-xl my-4 animate-fade-in">
        <div className="flex items-center justify-between px-6 py-5 border-b border-border bg-gradient-to-r from-brown-500 to-brown-700 rounded-t-2xl">
          ...
          <button onClick={onClose} className="p-2 rounded-lg hover:bg-surface/20 text-white transition-colors"><X size={20} /></button>
        </div>
              ...
              {state.employees.filter(e => e.status === 'prarambh').map(emp => (
                <label
                  ...
                  className={`flex items-center gap-3 p-3 rounded-xl border-2 cursor-pointer transition-all duration-200 ${
                    selectedEmployeeId === emp.id ? 'border-brown-500 bg-surface-elevated' : 'border-border hover:border-border'
                  }`}
                >
                  ...
                  {selectedEmployeeId === emp.id && <CheckCircle size={14} className="text-text-secondary flex-shrink-0" />}
                </label>
              ))}
            </div>
          </div>
              ...
              {tasks.map((task, i) => (
                <label
                  ...
                  className={`flex items-start gap-3 p-3 rounded-xl border-2 cursor-pointer transition-all duration-200 ${
                    selectedTaskIds.includes(i) ? 'border-brown-400 bg-surface-elevated' : 'border-border hover:border-border'
                  }`}
                >
                  <div className={`w-5 h-5 rounded-md border-2 flex items-center justify-center flex-shrink-0 mt-0.5 transition-all duration-200 ${
                    selectedTaskIds.includes(i) ? 'bg-brown-500 border-brown-500' : 'border-border'
                  }`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold text-text-primary text-sm leading-snug">{task.title}</p>
