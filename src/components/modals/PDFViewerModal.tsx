...
export default function PDFViewerModal({ doc, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4" onClick={onClose}>
      ...
      <div
        className={`relative bg-surface rounded-2xl shadow-2xl flex flex-col transition-all duration-200 ${
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-3.5 bg-gradient-to-r from-brown-600 to-brown-800 rounded-t-2xl flex-shrink-0">
          <div className="flex items-center gap-3 min-w-0">
            <div className="w-8 h-8 bg-surface/20 rounded-lg flex items-center justify-center flex-shrink-0">
          ...
          <div className="flex items-center gap-1.5 flex-shrink-0">
            ...
            {hasFile && (
              <button
                ...
                className="p-1.5 rounded-lg hover:bg-surface/20 text-white transition-colors"
                title="Download file"
              >
                <Download size={15} />
              </button>
        ...
        {/* ── Body ── */}
        <div className="flex-1 overflow-hidden rounded-b-2xl bg-surface">
          ...
          {hasFile && canText && textBody !== null && (
            <div className="h-full overflow-y-auto p-5">
              <div className="max-w-3xl mx-auto bg-surface rounded-xl shadow-sm border border-border">
                <div className="bg-gradient-to-br from-brown-50 to-blue-50 rounded-t-xl px-6 py-4 border-b border-border flex items-center gap-3">
                  <div className="w-10 h-10 bg-blue-500/10 rounded-xl flex items-center justify-center border border-blue-100">
                ...
                <pre className="px-6 py-5 text-xs text-text-primary leading-relaxed whitespace-pre-wrap font-mono overflow-x-auto">
                  {textBody || '(empty file)'}
                </pre>
              </div>
            </div>
          ...
          {hasFile && !canIframe && !canText && (
            <div className="h-full flex flex-col items-center justify-center gap-5 p-8">
              <div className="w-16 h-16 bg-surface-elevated/50 rounded-2xl flex items-center justify-center">
                <FileText size={28} className="text-text-secondary" />
              </div>
              <div className="text-center">
                <p className="font-bold text-text-primary text-lg mb-1">{doc.name}</p>
                <p className="text-sm text-text-secondary mb-1">{fileType} files cannot be previewed in the browser.</p>
                <p className="text-xs text-text-secondary/70">Download the file to open it in its native application.</p>
              </div>
              <button
                onClick={() => downloadFile(doc.fileData!, `${doc.name}.${fileType.toLowerCase()}`)}
          ...
          {!hasFile && doc.taskCount === undefined && (
            <div className="h-full flex flex-col items-center justify-center gap-4 p-8">
              ...
              <div className="text-center">
                <p className="font-bold text-text-primary mb-1">File preview not available</p>
                <p className="text-sm text-text-secondary mb-1">
                  The file content was cleared when the page was reloaded.
                </p>
                <p className="text-xs text-text-secondary/70">Re-upload the document to preview it again.</p>
              </div>
              <button
                onClick={onClose}
                className="flex items-center gap-2 text-sm text-text-secondary border border-border bg-surface px-4 py-2 rounded-xl hover:bg-surface-elevated transition-colors"
              >
          ...
          {!hasFile && doc.taskCount !== undefined && (
            <div className="h-full overflow-y-auto p-5">
              <div className="max-w-2xl mx-auto bg-surface rounded-xl shadow-sm border border-border">
                <div className="bg-gradient-to-br from-brown-50 to-blue-50 rounded-t-xl px-8 py-6 border-b border-border">
                  <div className="flex items-start gap-4">
                    <div className="w-14 h-14 bg-red-500/10 rounded-xl flex items-center justify-center border border-red-500/20 flex-shrink-0">
                      <BookOpen size={24} className="text-red-500" />
                    </div>
                    <div className="flex-1">
                      <h2 className="font-bold text-text-primary text-xl leading-tight mb-1">{doc.name}</h2>
                      <div className="flex flex-wrap items-center gap-3 text-xs text-text-secondary mt-2">
                        ...
                        <span className="bg-surface-elevated/50 text-text-secondary px-2 py-0.5 rounded-full font-medium">{doc.type}</span>
                        {doc.taskCount && (
                          <span className="bg-blue-100 text-blue-400 px-2 py-0.5 rounded-full font-medium">{doc.taskCount} tasks extracted</span>
                        )}
                      </div>
                    </div>
                  </div>
                ...
                <div className="px-8 py-6 space-y-4">
                  <h3 className="text-xs font-bold text-text-secondary/70 uppercase tracking-widest">Document Contents</h3>
                  {paragraphs.map((para, i) => (
                    <p key={i} className="text-sm text-text-primary leading-relaxed text-justify">{para}</p>
                  ))}
                </div>
                <div className="px-8 py-4 border-t border-border bg-surface-elevated rounded-b-xl flex items-center justify-between">
                  <p className="text-xs text-text-secondary/70">Preview Mode · Document content shown for reference</p>
                  <div className="flex gap-1">
                    {[1, 2, 3].map(p => (
                      <div key={p} className={`w-1.5 h-1.5 rounded-full ${p === 1 ? 'bg-brown-500' : 'bg-surface-elevated'}`} />
                    ))}
                  </div>
                </div>
              </div>
