  ...
  return (
    <div className="min-h-screen">
      <div className="max-w-7xl mx-auto px-4 py-6 space-y-6">
        ...
        {/* Tabs */}
        <div className="flex overflow-x-auto gap-1 bg-surface-elevated border border-border rounded-xl p-1">
          {TABS.map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              className={`flex-shrink-0 px-5 py-2.5 rounded-lg text-sm font-semibold transition-all duration-200 ${activeTab === tab ? 'bg-brown-500 text-white shadow-sm' : 'text-text-secondary hover:bg-surface'}`}>
              {tab}
            </button>
          ))}
        </div>
        ...
        {activeTab === 'Overview' && (
          <div className="grid lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 space-y-5">
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <button onClick={() => setShowAIChat(true)}
                      className="bg-surface-elevated text-purple-900 font-bold px-5 py-2.5 rounded-xl hover:bg-purple-50 transition-colors text-sm flex items-center gap-2">
              ...
              <div className="card">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="font-bold text-text-primary flex items-center gap-2"><ListChecks size={18} />Recently Assigned Tasks</h3>
                  <button onClick={() => setActiveTab('Employees & Tasks')} className="text-xs text-text-secondary hover:text-text-primary font-medium flex items-center gap-1">
                ...
                <div className="space-y-2">
                  {state.tasks.slice(-6).reverse().map(task => {
                    ...
                    return (
                      <div key={task.id} className="flex items-center gap-3 p-3 rounded-xl border border-border hover:bg-brown-50/50 transition-colors">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-text-secondary">{emp?.name ?? 'Unknown'} · by {task.assignedByName}</p>
                        </div>
                  ...
                  {state.tasks.length === 0 && (
                    <p className="text-center text-text-secondary text-sm py-6">No tasks assigned yet. Use the AI chat to generate tasks!</p>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="space-y-5">
              <div className="card">
                <h3 className="font-bold text-text-primary mb-4 flex items-center gap-2"><FileText size={16} />Documents</h3>
                <div className="space-y-2">
                  {state.documents.slice(0, 5).map(doc => (
                    <button
                      key={doc.id}
                      onClick={() => { setSelectedDocForChat(doc.id); setShowAIChat(true) }}
                      className="w-full flex items-center gap-3 p-2.5 rounded-lg hover:bg-surface transition-colors group text-left"
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-text-secondary">{doc.type} · {doc.size}</p>
                      </div>
                      <Bot size={14} className="text-text-secondary group-hover:text-text-secondary transition-colors flex-shrink-0" />
                    </button>
                  ))}
                </div>
                <button onClick={() => setShowAIChat(true)} className="w-full mt-3 text-xs font-semibold text-text-secondary border border-border py-2 rounded-lg hover:bg-surface transition-colors flex items-center justify-center gap-1">
                  <Upload size={12} /> Upload & Process New Document
                </button>
              </div>

              <div className="card">
                <h3 className="font-bold text-text-primary mb-4">Mentor Overview</h3>
                    ...
                    return (
                      <div key={mentor.id} className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-text-secondary">{menteeCount} mentee{menteeCount !== 1 ? 's' : ''}</p>
                        </div>
                      </div>
                    )
                  })}
                  ...
                  <div key={emp.id} className="card hover:shadow-md transition-all duration-200">
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 flex-wrap">
                          <h4 className="font-bold text-text-primary">{emp.name}</h4>
                          {emp.risk === 'high' && <AlertTriangle size={14} className="text-red-500" />}
                        </div>
                        <p className="text-sm text-text-secondary">{emp.role} · Mentor: {getMentorName(emp.mentorId)}</p>
                        <div className="flex items-center gap-3 mt-2">
                          <div className="w-32 progress-bar">
                            <div className="progress-fill" style={{ width: `${emp.progress}%` }} />
                          </div>
                          <span className="text-xs text-text-secondary">{emp.progress}% · Day {emp.day}/{emp.totalDays}</span>
                    ...
                    {isExpanded && myTasks.length > 0 && (
                      <div className="mt-4 pt-4 border-t border-border space-y-2 animate-fade-in">
                        <p className="text-xs font-semibold text-text-secondary mb-2">Assigned Tasks ({myTasks.length})</p>
                        {myTasks.map(task => (
                          <div key={task.id} className="flex items-center gap-3 p-2.5 rounded-lg bg-surface border border-border">
                            <div className={`w-2 h-2 rounded-full flex-shrink-0 ${task.status === 'done' ? 'bg-green-500' : task.status === 'in-progress' ? 'bg-brown-500' : 'bg-brown-300'}`} />
                            <p className="text-sm text-text-primary flex-1 truncate">{task.title}</p>
                            <div className="flex items-center gap-2 flex-shrink-0">
                              <span className="badge-brown text-xs py-0.5">{task.category}</span>
                              <span className="flex items-center gap-1 text-xs text-text-secondary"><Clock size={10} />{task.estimatedTime}</span>
                            </div>
                          </div>
                        ))}
                      </div>
                    ...
                    {isExpanded && myTasks.length === 0 && (
                      <div className="mt-4 pt-4 border-t border-border text-center py-4 text-text-secondary text-sm animate-fade-in">
                        No tasks assigned yet. <button onClick={() => { setSelectedEmployee(emp.id); setShowAIChat(true) }} className="text-text-secondary underline">Generate with AI</button>
                      </div>
                    )}
                  </div>
                )
          ...
          <div className="space-y-5">
            ...
            <div className="card p-0 overflow-hidden">
              <div className="px-6 py-4 border-b border-border flex justify-between items-center">
                <h3 className="font-bold text-text-primary">All Documents</h3>
                ...
                {state.documents.map(doc => (
                  <div key={doc.id} className="flex items-center gap-4 px-6 py-4 hover:bg-brown-50/50 transition-colors">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-text-secondary">{doc.type} · {doc.size} · Uploaded {doc.date}</p>
                    </div>
                    <div className="flex items-center gap-3 flex-shrink-0">
                      ...
                      <button onClick={() => { setSelectedDocForChat(doc.id); setShowAIChat(true) }}
                        className="text-xs font-semibold text-purple-600 border border-purple-200 bg-surface-elevated px-3 py-1.5 rounded-lg hover:bg-purple-50 transition-colors flex items-center gap-1">
        ...
        {activeTab === 'Analytics' && (
          <div className="grid lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 card">
              <h3 className="font-bold text-text-primary mb-5 flex items-center gap-2"><BarChart3 size={18} />Task Completion by Employee</h3>
              <div className="space-y-4">
                    ...
                    <div key={emp.id} className="flex items-center gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between mb-1">
                          <span className="text-sm font-semibold text-text-primary truncate">{emp.name}</span>
                          <span className="text-xs text-text-secondary ml-2 flex-shrink-0">{done}/{myTasks.length} tasks · {pct}%</span>
                        </div>
            ...
            <div className="space-y-4">
              {[
                { label: 'Tasks by Admin', value: state.tasks.filter(t => t.assignedBy === 'admin').length, color: 'text-text-secondary bg-surface' },
