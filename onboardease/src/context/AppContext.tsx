...
// ─── Clear any stale localStorage data from the old storage-based approach ───
try {
  localStorage.removeItem('prarambh_state')
  localStorage.removeItem('prarambh_version')
} catch { /* ignore */ }
...
function reducer(state: AppState, action: Action): AppState {
  switch (action.type) {
    ...
    case 'RESTORE_STATE':
      // Merge API state without overwriting the session role/userId (those come from the URL)
      return {
        ...state,
        ...action.payload,
...
export function AppProvider({ children }: { children: ReactNode }) {
  // ── Default (empty) state — API hydration happens after mount ──────────────
  ...
  // dbReady guards API writes so we don't echo seed data back during hydration
  const [dbReady, setDbReady] = useState(false)
  const dbReadyRef = useRef(false)

  // Refs for tracking reducer-generated side-effects (notifications, task
  // mutations from subtasks/feedback/reorder, message read-receipts)
  const prevNotifsRef = useRef<Notification[]>([])
  const prevTasksRef  = useRef<Task[]>([])
  const prevMsgsRef   = useRef<ChatMessage[]>([])

  // ── On mount: hydrate state from SQLite via API ────────────────────────────
  useEffect(() => {
    dataApi.getState()
      .then(saved => {
        dispatch({ type: 'RESTORE_STATE', payload: saved })
        // Sync refs so change-tracking doesn't fire spurious updates
        prevNotifsRef.current = (saved.notifications  as Notification[])  ?? []
        prevTasksRef.current  = (saved.tasks          as Task[])           ?? []
        prevMsgsRef.current   = (saved.chatMessages   as ChatMessage[])    ?? []
        dbReadyRef.current    = true
        setDbReady(true)
      })
      .catch(() => {
        // API unavailable — fall back to seed data already in state
        dbReadyRef.current = true
        setDbReady(true)
      })
  ...
  // ── Sync new / updated notifications (auto-created inside reducer) ─────────
  useEffect(() => {
    if (!dbReady) return
    const prev = prevNotifsRef.current
    const curr = state.notifications
    // New notifications
    curr.filter(n => !prev.find(p => p.id === n.id))
        .forEach(n => dataApi.createNotification(n).catch(() => {}))
    // Updated readBy on existing notifications
    curr.forEach(n => {
      const p = prev.find(p => p.id === n.id)
      if (p && JSON.stringify(p.readBy) !== JSON.stringify(n.readBy))
        dataApi.updateNotification(n.id, { readBy: n.readBy, read: n.read }).catch(() => {})
    })
    prevNotifsRef.current = curr
  }, [state.notifications, dbReady])

  // ── Sync task mutations done inside the reducer ────────────────────────────
  // Catches: UPDATE_SUBTASK_STATUS, ADD_TASK_FEEDBACK, REORDER_TASK
  useEffect(() => {
    if (!dbReady) return
    ...
    state.tasks.forEach(t => {
      ...
      if (p && JSON.stringify(p) !== JSON.stringify(t))
        dataApi.updateTask(t.id, t).catch(() => {})
    })
    prevTasksRef.current = state.tasks
  }, [state.tasks, dbReady])

  // ── Sync message read-receipts mutated inside the reducer ─────────────────
  useEffect(() => {
    if (!dbReady) return
    const prev = prevMsgsRef.current
    state.chatMessages.forEach(m => {
      const p = prev.find(p => p.id === m.id)
      if (p && JSON.stringify(p.readBy) !== JSON.stringify(m.readBy))
        dataApi.updateMessage(m.id, { readBy: m.readBy }).catch(() => {})
    })
    prevMsgsRef.current = state.chatMessages
  }, [state.chatMessages, dbReady])

  // ── API-synced dispatch ────────────────────────────────────────────────────
  // Updates local state immediately (optimistic UI) then persists to backend.
  const apiDispatch = useCallback((action: Action) => {
    dispatch(action)
    if (!dbReadyRef.current) return
    switch (action.type) {
      case 'ADD_EMPLOYEE':
        dataApi.createEmployee(action.payload).catch(() => {})
        break
      case 'REMOVE_EMPLOYEE':
        dataApi.deleteEmployee(action.payload.id).catch(() => {})
        break
      case 'UPDATE_EMPLOYEE_BIO':
        dataApi.updateEmployee(action.payload.id, { bio: action.payload.bio }).catch(() => {})
        break
      case 'UPDATE_EMPLOYEE_RESUME':
        dataApi.updateEmployee(action.payload.id, {
          resumeFileName: action.payload.resumeFileName,
          resumeContent:  action.payload.resumeContent,
        }).catch(() => {})
        break
      case 'ADD_TASK':
        dataApi.createTask(action.payload).catch(() => {})
        break
      case 'ADD_TASKS':
        dataApi.createTasks(action.payload).catch(() => {})
        break
      case 'REMOVE_TASK':
        dataApi.deleteTask(action.payload.id).catch(() => {})
        break
      case 'UPDATE_TASK':
        dataApi.updateTask(action.payload.id, action.payload.updates).catch(() => {})
        break
      case 'UPDATE_TASK_STATUS':
        dataApi.updateTask(action.payload.id, { status: action.payload.status }).catch(() => {})
        break
      case 'UPDATE_TASK_INPUT':
        dataApi.updateTask(action.payload.taskId, { inputValue: action.payload.inputValue }).catch(() => {})
        break
      case 'ADD_DOCUMENT':
        dataApi.createDocument(action.payload).catch(() => {})
        break
      case 'REMOVE_DOCUMENT':
        dataApi.deleteDocument(action.payload.id).catch(() => {})
        break
      case 'ADD_MENTOR':
        dataApi.createMentor(action.payload).catch(() => {})
        break
      case 'REMOVE_MENTOR':
        dataApi.deleteMentor(action.payload.id).catch(() => {})
        break
      case 'CREATE_CONVERSATION':
        dataApi.createConversation(action.payload).catch(() => {})
        break
      case 'DELETE_CONVERSATION':
        dataApi.deleteConversation(action.payload.id).catch(() => {})
        break
      case 'ADD_CHAT_MESSAGE':
        dataApi.createMessage(action.payload).catch(() => {})
        break
      case 'SCHEDULE_MEETING':
        dataApi.createMeeting(action.payload).catch(() => {})
        break
      case 'UPDATE_COMPANY_SETTINGS':
        dataApi.updateSettings(action.payload).catch(() => {})
        break
    }
  }, []) // stable — reads dbReadyRef via ref, not state
